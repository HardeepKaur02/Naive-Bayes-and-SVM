...
##### function to compute the pairwise kernels efficiently
...
def Gaussian_SVM(d1, d2, gamma, n, C, train_file_name,test_file_name ):
    ...
    y = np.concatenate((np.ones((nd1,1))*(-1.0),np.ones((nd2,1))*1.0),axis=0)
    K = KernelMat(X)
    ...
    output_file = "Gaussian_SV" + str(int(d1)) + str(int(d2)) + ".csv"
    ...
    for row in csvReader:
        ...
        if row_val[-1] == d1 or row_val[-1] == d2:
            ...
            for i in range(m):
                if alpha[i] > 0.000001:
                    k = np.exp(-0.05 * (np.linalg.norm(X[i] - test_x))**2 )
                    val += alpha[i] * y[i] * k
            pred = val + b_intercept
            if pred > 0.0:
                pred = 1
            else:
                pred = -1
